...
`.github/dependabot.yml` checks Swift packages (including Sparkle) on weekdays at 06:00 Europe/Zurich and GitHub Actions weekly on Monday. Dependabot updates the exact version in `Package.swift` and the committed `Package.resolved` in a pull request. Keep the exact pin: dependency changes should be visible in review, not silently selected during release packaging. Version-update scheduling starts once this configuration reaches `main`; repository Dependabot alerts/security updates are enabled separately in GitHub's security settings.

CI runs for every PR targeting `main`, including Dependabot PRs, and after merges. It builds using the committed resolved versions, runs the Swift tests, verifies framework embedding/signing in the local package, and exercises signed-feed/download validation with a disposable test key. It needs no production signing or notarization secrets and uses a read-only token. Updates require normal review and merging; no automatic merge or release publishing is configured.

For a Sparkle update, inspect its release notes/API changes and review any changed framework/tool layout. Before shipping, rerun Developer ID packaging/notarization and the isolated update install/relaunch and active-connection cancellation checks. Passing ad-hoc CI is not a substitute for that release rehearsal. See [GitHub's Swift support](https://docs.github.com/en/code-security/reference/supply-chain-security/supported-ecosystems-and-repositories#swift) and [Dependabot's Actions restrictions](https://docs.github.com/en/code-security/reference/supply-chain-security/troubleshoot-dependabot/dependabot-on-actions).
